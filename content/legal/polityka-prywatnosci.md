# Polityka Prywatności LessManual
...
Niniejsza Polityka Prywatności (dalej: **"Polityka"**) określa zasady przetwarzania danych osobowych przez LessManual w związku z:
- Korzystaniem z serwisu internetowego LessManual
...
- **Administrator** – Bartłomiej Chudzik, prowadzący działalność gospodarczą pod firmą LessManual, z siedzibą w ul. Długa 33, 05-530 Cendrowice, Polska, NIP: 1231589909, email: kontakt@LessManual
...
- **Serwis** – serwis internetowy LessManual dostępny pod domeną https://LessManual
...
**Bartłomiej Chudzik**
LessManual
ul. Długa 33, 05-530 Cendrowice, Polska
NIP: 1231589909
...
**Źródło:** automatyczne zbieranie podczas korzystania z Serwisu (analityka Google Analytics - USA, wymaga zgody, cookies _ga, _gid, _gat)
...
**Źródło:** systemy CRM (Notion, Supabase), korespondencja handlowa
...
- Napisz do nas: kontakt@LessManual
...
**Narzędzie:** **Google Analytics 4** (hosting USA, Google LLC, wymaga zgody użytkownika, cookies analityczne: _ga, _gid, _gat, anonimizacja IP).

**Podstawa prawna:** Art. 6 ust. 1 lit. a RODO (zgoda użytkownika - udzielana przez Cookie Banner).

**Zakres danych:** IP (anonimizowany), cookies (_ga: identyfikator użytkownika, _gid: identyfikator sesji, _gat: throttling), user agent, referrer, odwiedzone strony, czas sesji. **BRAK cross-site tracking poza Google**, **BRAK fingerprinting**.

**Okres:** 14 miesięcy (cookies _ga), 24 godziny (_gid), 1 minuta (_gat). Dane w Google Analytics: 14 miesięcy.
...
**Zakres danych:** IP, logi dostępu (timestamp, endpoint, status HTTP), logi aplikacji.
...
- **Supabase** (Sztokholm, Szwecja) - hosting bazy danych (PostgreSQL), EU region
- **n8n** (self-hosted, Niemcy) - workflows automatyzacji

**AI i automatyzacja:**
- **Anthropic** (USA) - Claude API (przetwarzanie języka naturalnego dla ChatBotów)
- **Google Gemini** (USA, Google LLC) - API przetwarzania języka naturalnego i generowania treści
...
**CRM i dane:**
- **Notion** (USA/EU) - baza danych leadów ze strony internetowej
- **Supabase** (Sztokholm, Szwecja) - baza danych leadów i klientów
...
- Vercel (hosting frontend - EU region preferred)
- Anthropic (Claude API)
- Google Gemini (AI API)
- ElevenLabs (VoiceAgent API)
- Google Analytics (analityka)
- Notion (baza danych leadów - EU/USA hybrid)
- Cal.com (booking - self-hosted EU option available)

**Procesorzy w UE:**
- Supabase (Sztokholm, Szwecja - baza danych)
- n8n (self-hosted, Niemcy - workflows)
...
- Google LLC (Gemini, Analytics) - DPF certified
- ElevenLabs - DPF certified
...
- Dane at rest: **AES-256** (Supabase, Vercel)
- Klucze szyfrujące przechowywane w EU (Supabase Stockholm region)

**2. Minimalizacja danych:**
- Przekazywanie tylko danych niezbędnych do świadczenia Usług
- Pseudonimizacja danych wrażliwych (np. IP → hashed IP, anonimizacja IP w Google Analytics)
- Agregacja danych analitycznych (Google Analytics - USA hosting, anonimizacja IP, brak eksportu surowych danych osobowych)

**3. Wybór regionów EU (gdzie dostępne):**
- Supabase: **Stockholm, Szwecja** (EU region)
- Vercel: **EU region deployment** (preferred)
- n8n: **self-hosted Niemcy**
...
**Kontakt w sprawie sprzeciwu:** kontakt@LessManual
...
| **Logi bezpieczeństwa (aplikacji)** | 90 dni LUB do czasu zakończenia postępowania (jeśli wykryto atak) | Art. 6 ust. 1 lit. f RODO |
...
- Logi bezpieczeństwa: **automatyczne usunięcie po 90 dniach** (konfiguracja retencji)
...
6. **Dane zebrano w związku z oferowaniem usług społeczeństwa informacyjnego dzieciom** (art. 8 RODO)
   - *Nie dotyczy LessManual (B2B, brak usług dla dzieci)*
...
3. **Archiwizacja w interesie publicznym, badania naukowe, cele statystyczne**
   - *Nie dotyczy LessManual (brak badań naukowych)*
...
- Dane Administratora (LessManual, Bartłomiej Chudzik)
...
**📧 Email (preferowany):** kontakt@LessManual
**✉️ Poczta:** Bartłomiej Chudzik, LessManual, ul. Długa 33, 05-530 Cendrowice, Polska
...
- Szyfrowanie dysków (full disk encryption) - Vercel, Supabase
...
- **Vercel Analytics** - monitoring ruchu i wydajności
- **Supabase Realtime** - monitoring zapytań do bazy danych, slow queries

**Alerty bezpieczeństwa:**
- **Email** - natychmiastowe powiadomienia o:
...
- Raportowanie podejrzanych emaili (dedykowany email)
...
- Link "Unsubscribe" w emailu LUB kontakt: kontakt@LessManual
...
**Jak to działa:**
- Google Analytics 4 (USA, Google LLC, GDPR-compliant z odpowiednimi zabezpieczeniami):
  - Cookies analityczne: _ga (identyfikator użytkownika, 14 mies.), _gid (sesja, 24h), _gat (throttling, 1 min)
  - Które podstrony są najczęściej odwiedzane
  - Średni czas spędzony na stronie
  - Źródło ruchu (Google, LinkedIn, direct)
  - Anonimizacja IP (brak pełnego adresu IP)
- Agregacja danych (np. "80% użytkowników przychodzi z LinkedIn")

**Podstawa prawna:**
- **Art. 6 ust. 1 lit. a RODO** (zgoda użytkownika - udzielana przez Cookie Banner)

**Prawo sprzeciwu:**
- **TAK** - odrzuć cookies analityczne w Cookie Banner, wycofaj zgodę w ustawieniach cookies lub użyj Google Analytics Opt-out Add-on
...
**Prawo sprzeciwu:**
- **TAK** - kontakt: kontakt@LessManual
...
**LessManual NIE stosuje takich decyzji:**
...
**[Polityka Cookies LessManual](/legal/polityka-cookies)**

### 11.2 Podsumowanie (TL;DR)

**LessManual stosuje minimalistyczne podejście do cookies:**
...
3. **Możliwość sprzeciwu** - jeśli nie zgadzasz się z nowymi zasadami, możesz:
   - Wnieść sprzeciw (kontakt: kontakt@LessManual)
...
**Kontakt w sprawie Rejestru:** kontakt@LessManual
...
📧 **Email:** kontakt@LessManual
🌐 **Strona internetowa:** https://LessManual

**W sprawach dotyczących ochrony danych osobowych:** kontakt@LessManual (Subject: "RODO - [Twoje imię i nazwisko]")
...
- Publikację niniejszej Polityki na stronie https://LessManual/legal/polityka-prywatnosci
...
**Jurysdykcja:** W przypadku sporów dotyczących ochrony danych osobowych, właściwy jest **sąd polski** zgodnie z miejscem zamieszkania/siedziby Administratora (ul. Długa 33, 05-530 Cendrowice, Polska) lub miejscem zamieszkania osoby, której dane dotyczą (zgodnie z art. 79 ust. 2 RODO - osoba ma wybór).
...
|------------------------------------|-------------------------------------------------------------------------------------------------------------|
| **Administrator**                  | Bartłomiej Chudzik, LessManual, ul. Długa 33, 05-530 Cendrowice, Polska                                                      |
...
| **Odbiorcy danych**                | Procesorzy (Supabase, Vercel, Anthropic, Google Gemini, ElevenLabs, Google Analytics, Notion, Cal.com), organy publiczne (na żądanie) |
...
| **Cookies**                        | Minimalistyczne: funkcjonalne (sb-auth, XSRF), analityczne Google Analytics (_ga, _gid, _gat - wymaga zgody), marketingowe tylko za zgodą |
...
**📧 Email:** kontakt@LessManual (Subject: "RODO - [Twoje pytanie]")
**✉️ Poczta:** Bartłomiej Chudzik, LessManual, ul. Długa 33, 05-530 Cendrowice, Polska
